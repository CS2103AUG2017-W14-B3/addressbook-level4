...
This feature is implemented to provide users the ability to email their contacts using the users' default mail client
to launch an email application which will contain the email addresses of contacts specified by users. Users can compose or edit the email using their own default email application before sending it. +

The java `Desktop` class allows java application to launch default email applications registered on the users' native desktop to handle a Uniform Resource Indentifier `URI`.
In this case, `URI` is created as a reference to the mail command scheme and email addresses to mail to.
...
From the diagram above, when users enter the command `email 1` to email one or more person in their address book.
...
public CommandResult execute() throws CommandException {
        ...
        for (Index targetIndex : targetIndexes) {
            if (targetIndex.getZeroBased() >= lastShownList.size()) {
                //Throw exception due to invalid index after checking
            }
            //Append the address of the person to addresses and do others
            address.append(" ");
            addresses.append(personToEmail.getEmail.toString());
        }
        //Add a comma between email addresses
...
//rest of methods in MainWindow

URI mailTo = new URI(EMAIL_URI_PREFIX + allEmailAddresses);
    //Checks if user native Desktop applications are supported
...
In addition, users can also edit their recipients field to include carbon copy or blind carbon copy before sending out
