...
:repoURL: https://github.com/JunQuann/main.git
...
|`* * *` |user |use acronyms instead of the full command |use the app more efficiently
...
|`* * *` |user |import contacts from Google Contacts/iCloud |populate the app without having to add manually
...
|`* * *` |user |view my favourite contacts immediately |save time by not having to find them
...
|`* *` |user |add display picture for my contact |can easily identify them
...
1.  User requests to list persons
2.  AddressBook shows a list of persons
3.  User selects persons to email
4.  User enters keyboard shortcut to email
5.  AddressBook opens user's mail application with persons' emails put into the "To:" field
+
Use case ends.
...
4.  AddressBook opens user's default browser with the page of the desired person
+
Use case ends.

[discrete]
=== Use case: Import contacts from iCloud/Google

*MSS*

1.  User requests to import a .CSV file
2.  AddressBook shows user amount of newly added contacts
3.  AddressBook shows updated contact list to the user
+
Use case ends.
...
4.  AddressBook creates a vCard file in the application directory
+
Use case ends.
...
1.  User requests to list persons
2.  AddressBook shows a list of persons
3.  User decides on which tag to search for
4.  User requests to find persons using desired tag
5.  AddressBook shows the list of persons whose tags match the user's tag
+
Use case ends.
...
1.  User requests to list persons with same name or number
2.  User selects the persons that he wants to keep
3.  Addressbook deletes the rest of the persons that were not selected
...
[none]
* 2a.   Command already has a key aliased to it
* 2b.   Choose to replace the key aliased

+
Use case ends

[none]
* 3a.   Key already aliased to other command
* 3b.   Choose to replace the command aliased
...
.  Should not crash more than once while starting the AddressBook.
...
.  95% of users who did not use the Address Book before should be familiar with it after reading the UserGuide.
.  Address Book user should not take more than 2 second to respond after a command has been entered.
.  The information stored in the user local storage should not be accessible by other users.
...
Aliases
....
Shortcuts for the default commands so that the user can customize AddressBook to
their liking.
....
