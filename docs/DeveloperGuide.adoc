...
* Define its _link:#api[API]_ in an `interface` with the same name as the Component.
...
image::LogicClassDiagram.png[width="800"]
_Figure 4.3a. Structure of the Logic Component_

The command execution can affect the `Model` (e.g. adding a person) and/or raise events.
The result of the command execution is encapsulated as a `CommandResult` object which is passed back to the `Ui`.

The figure below shows the finer details concerning `XYZCommand` and `Command` in Figure 4.3b.

image::LogicCommandClassDiagram.png[width="800"]
_Figure 4.3b. Structure of Commands in the Logic Component._
...
_Figure 4.3c. Interactions Inside the Logic Component for the `delete 1` Command_
...
_Figure 4.4. Structure of the Model Component_
...
_Figure 4.5. Structure of the Storage Component_
...
The design considerations are also discussed at the end of each feature.
...
image::LogicCommandClassDiagram.png[width="800"]
_Figure 5.1a. Logic Command Class Diagram_
...
_Figure 5.1b. Undo Redo Starting Stack Diagram_

As the user continues to use the program, more commands are added into the `undoStack`. For example, the user may execute `add n/David ...` to add a new person.

image::UndoRedoNewCommand1StackDiagram.png[width="800"]
_Figure 5.1c. Undo Redo New Command Stack Diagram_
...
_Figure 5.1d. Undo Redo Execute Undo Stack Diagram_
...
_Figure 5.1e. Undo Redo Sequence Diagram_
...
_Figure 5.1g. Undo Redo New Command Stack Diagram 3_

The following activity diagram summarize what happens inside the `UndoRedoStack` when a user executes a new command:

image::UndoRedoActivityDiagram.png[width="200"]
_Figure 5.1h. Undo Redo Activity Diagram_
...
_Figure 5.2a. Email Command Sequence Diagram_
...
_Figure 5.2b. Email Request Event Sequence Diagram_

The diagram above illustrates the event driven process of the `EmailCommand`. After `EmailCommand` has executed, it will post a new `EmailRequestEvent` and allows the UI to handle this event. In the UI, we are using java `Desktop` class to
...
The process when user enters the export command to the creation of the vCard file is summarized using the activity
diagram shown below.

image::ExportActivityDiagram.png[width="500"]
_Figure 5.4a. Export Activity Diagram_

image::vCardClassDiagram.png[width="500"]
_Figure 5.4b. Vcard Class Diagram_
...
_Figure 5.6. Tag Delete Sequence Diagram_
...
_Figure 5.7. Tag Edit Sequence Diagram_
...
_Figure 5.9. Social Sequence Diagram_
...
_Figure 5.10. Location Activity Diagram_
