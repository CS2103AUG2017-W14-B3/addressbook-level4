...
Change the Graphic User Interface (GUI) of HitMeUp
...
When the display picture of the person is edited or when the person is deleted, the image that is no longer in used should be deleted as well.

==== Justification

Images that are no longer in used will not use up space in the storage.

==== Implementation

When editing the display picture of a person or deleting a person, we can check the display picture path that is stored as the `DisplayPic` field and delete the image. +
However, this will cause the image to not render properly if we undo the `Edit` the `Delete` command. +
Possible solution is to store images that are no longer in use in a back up folder. Whenever the application closes, delete all the images in the back up folder.
